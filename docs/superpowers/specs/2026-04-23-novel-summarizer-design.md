...
| 连续 3 batch 失败 | 终止分析，`messagebox.showerror("网络或 API 异常")` |
...
1. `cd /Users/horace/playground/analyze-novel`
2. `git init && git branch -M main`
3. `git remote add origin https://github.com/huangshihao/analyze-novel.git`
4. 按 writing-plans 产出的实施计划一步步写代码
5. 全部跑通后首次 push 到 GitHub，Actions 自动出 exe
